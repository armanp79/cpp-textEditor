...
{
    run({ 'H', 'I', CTRL('M'), 'L', CTRL('I') ,CTRL('K') });
    expectCursor(2, 2);
    expectNoErrorMessage();
}
...
{
    run({ 'H', 'I', 'I', 'I','I','I','I','I', 'L', CTRL('U') ,CTRL('Y') });
    expectCursor(1, 1);
    expectNoErrorMessage();
}
...
{
    run({ 'H', 'I', 'I', 'I','I','I','I', 'L', CTRL('U') ,CTRL('Y'),CTRL('P') });
    expectCursor(1, 9);
    expectNoErrorMessage();
}


TEST_F(BooEditTests, makeNewLineAtBeginning)
{
    run({ 'H', 'E', 'L', 'L','O','W','O','R', 'L' ,CTRL('Y'), CTRL('M') });
    expectText({"", "HELLOWORL"});
    expectNoErrorMessage();
}

TEST_F(BooEditTests, undoNewLineAtBeginning)
{
    run({ 'H', 'E', 'L', 'L','O','W','O','R', 'L' ,CTRL('Y'), CTRL('M'), CTRL('Z')});
    expectText({"HELLOWORL"});
    expectNoErrorMessage();
}

TEST_F(BooEditTests, deleteLineAtBeginning)
{
    run({ 'H', 'E', 'L', 'L','O','W','O','R', 'L' ,CTRL('Y'), CTRL('D') });
    expectText({""});
    expectNoErrorMessage();
}

TEST_F(BooEditTests, backspaceAll)
{
    run({ 'H', 'E', 'L', 'L','O','W','O','R', 'L', CTRL('M'), 'H', CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H'),CTRL('H') });
    expectText({""});
    expectNoErrorMessage();
}
...
{
    run({ 'H',CTRL('M'), 'E', CTRL('M'), 'Y', CTRL('M'), CTRL('D'),CTRL('Z')  });
    expectText({"H", "E", "Y", ""});
    expectNoErrorMessage();
}

TEST_F(BooEditTests, undoNewDeleteLine)
{
    run({ 'H',CTRL('M'), 'E', CTRL('M'), 'Y', CTRL('M'), CTRL('D'),CTRL('Z'),CTRL('Z'),CTRL('Z'),CTRL('Z'),CTRL('Z'),CTRL('Z'),CTRL('Z')  });
    expectText({""});
    expectNoErrorMessage();
}
